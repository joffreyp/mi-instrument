...
class PlatformDriver(object, metaclass=META_LOGGER):
    ...
    def _driver_event(self, event_type, val=None):
        ...
        if event_type == DriverAsyncEvent.STATE_CHANGE:
            val = self.get_resource_state()

        elif event_type == DriverAsyncEvent.CONFIG_CHANGE:
            val = self.get_resource(DriverParameter.ALL)
    ...
    def _construct_fsm(self, states=PlatformDriverState,
        ...
        self._fsm.add_handler(PlatformDriverState.UNCONFIGURED, PlatformDriverEvent.CONFIGURE,
                              self._handler_unconfigured_configure)

        # DISCONNECTED state event handlers:
        self._fsm.add_handler(PlatformDriverState.DISCONNECTED, PlatformDriverEvent.CONNECT,
                              self._handler_disconnected_connect)
        self._fsm.add_handler(PlatformDriverState.DISCONNECTED, PlatformDriverEvent.DISCONNECT,
                              self._handler_disconnected_disconnect)

        # CONNECTED state event handlers:
        self._fsm.add_handler(PlatformDriverState.CONNECTED, PlatformDriverEvent.DISCONNECT,
                              self._handler_connected_disconnect)
        self._fsm.add_handler(PlatformDriverState.CONNECTED, PlatformDriverEvent.CONNECTION_LOST,
        ...
        self._fsm.add_handler(PlatformDriverState.CONNECTED, PlatformDriverEvent.EXECUTE,
